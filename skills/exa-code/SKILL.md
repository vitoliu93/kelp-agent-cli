---
name: exa-code
description: MCP skill providing 2 tool(s): web_search_exa, get_code_context_exa
---

# exa-code

Tools available via MCP (http transport).
...
Returns: Clean text content from top search results, ready for LLM use.
  Run: `bun skills/exa-code/scripts/web_search_exa.ts '<json args>'`
  Input schema:
    {
      "type": "object",
      "properties": {
...
Returns: Relevant code and documentation, formatted for easy reading.
  Run: `bun skills/exa-code/scripts/get_code_context_exa.ts '<json args>'`
  Input schema:
    {
      "type": "object",
      "properties": {
